...
const MAIN_ADMIN_EMAIL = 'psycho12e4@gmail.com';
...
const ApexAdmin = {
    ...
    async _getScopeContext() {
        const currentEmail = this.normalizeIdentifier(await this._getCurrentSessionEmail());
        return {
            currentEmail,
            isMainAdmin: await this.isMainAdmin(currentEmail)
        };
    },

    _matchesActor(recordValue, currentEmail) {
        return this.normalizeIdentifier(recordValue) === this.normalizeIdentifier(currentEmail);
    },

    async isMainAdmin(identifier) {
        const email = this.normalizeIdentifier(identifier || await this._getCurrentSessionEmail());
        return email === MAIN_ADMIN_EMAIL;
    },

    async canGrantAdminAccess(identifier) {
        return this.isMainAdmin(identifier);
    },

    async canViewAuditLogs(identifier) {
        return this.isMainAdmin(identifier);
    },
    ...
    async addAdminUser(identifier, role = 'admin') {
        if (!(await this.canGrantAdminAccess())) {
            return { ok: false, error: 'Only the main admin can grant admin access.' };
        }
    ...
    async removeAdminUser(identifier) {
        if (!(await this.canGrantAdminAccess())) {
            return { ok: false, error: 'Only the main admin can remove admin access.' };
        }

        identifier = this.normalizeIdentifier(identifier);
        if (!identifier) return { ok: false, error: 'Email is required' };

        if (identifier === MAIN_ADMIN_EMAIL) {
            return { ok: false, error: 'The main admin account cannot be removed.' };
        }
    ...
    async listAdminUsers() {
        const scope = await this._getScopeContext();
        const rows = await this._loadAdminRows();
        const filteredRows = scope.isMainAdmin
            ? rows
            : rows.filter(entry => this._matchesActor(entry.email, scope.currentEmail) || this._matchesActor(entry.added_by, scope.currentEmail));
    ...
    async listAccessRequests() {
        ...
        try {
            const { data, error } = await db
                ...
                .select('id, email, reason, status, requested_by, resolved_by, created_at')
                .order('created_at', { ascending: false });
            if (!error && Array.isArray(data)) {
                const filtered = scope.isMainAdmin
                    ? data
                    : data.filter(entry => this._matchesActor(entry.resolved_by, scope.currentEmail) || this._matchesActor(entry.requested_by, scope.currentEmail));
                return filtered.map(entry => ({ ...entry, email: this.normalizeIdentifier(entry.email) }));
            }
        } catch (e) {
    ...
    async approveAccessRequest(id, email) {
        if (!(await this.canGrantAdminAccess())) {
            return { ok: false, error: 'Only the main admin can approve admin access.' };
        }

        if (!id) return { ok: false, error: 'Request id is required' };
        const identifier = this.normalizeIdentifier(email);
        const actorEmail = this.normalizeIdentifier(await this._getCurrentSessionEmail());
        try {
            const { error } = await db.from('admin_access_requests').update({
                status: 'approved',
                resolved_by: actorEmail || null
            }).eq('id', id);
            if (error) throw error;
        } catch (e) {
    ...
    async rejectAccessRequest(id, email) {
        if (!(await this.canGrantAdminAccess())) {
            return { ok: false, error: 'Only the main admin can reject admin access requests.' };
        }
    ...
    async deleteAccessRequest(id) {
        if (!id) return { ok: false, error: 'Request id is required' };
        try {
            const { error } = await db.from('admin_access_requests').delete().eq('id', id);
            if (error) throw error;
        } catch (e) {
            console.warn('[Admin] Could not delete access request:', e);
            return { ok: false, error: 'Could not delete access request.' };
        }
        return { ok: true };
    },
    ...
    async listAuditLogs(limit = 20) {
        ...
        try {
            const { data, error } = await db
                ...
                .limit(limit);
            if (!error && Array.isArray(data)) {
                return scope.isMainAdmin
                    ? data
                    : data.filter(entry => this._matchesActor(entry.actor_email, scope.currentEmail));
            }
        } catch (e) {
        ...
        if (!scope.isMainAdmin) {
            try {
                const { data, error } = await db
                    ...
                    .limit(1);
                if (error) throw error;
                if (!data || data.length === 0) return { ok: false, error: 'Block record not found.' };
                if (this.normalizeIdentifier(data[0].blocked_by) !== scope.currentEmail) {
                    return { ok: false, error: 'You can only unblock users you personally blocked.' };
                }
            } catch (e) {
                return { ok: false, error: 'Could not verify ownership of this block.' };
            }
        }
    ...
    async listBlockedUsers() {
        ...
        try {
            ...
            if (!error && Array.isArray(data)) {
                const filtered = scope.isMainAdmin
                    ? data
                    : data.filter(entry => this._matchesActor(entry.blocked_by, scope.currentEmail));
                return filtered.map(entry => ({ ...entry, email: this.normalizeIdentifier(entry.email) }));
            }
        } catch (e) {
    ...
    async listPremiumMembers() {
        ...
        try {
            const { data, error } = await db
                ...
                .order('created_at', { ascending: false });
            if (!error && Array.isArray(data)) {
                testUsers = scope.isMainAdmin
                    ? data
                    : data.filter(entry => this._matchesActor(entry.granted_by, scope.currentEmail));
            }
        } catch (e) {
            console.warn('[Admin] Could not load admin grants from Supabase:', e);
        }

        return scope.isMainAdmin ? mergeMembers(memberships, testUsers, localMembers) : mergeMembers([], testUsers, []);
    },
    ...
    async removeTestUser(identifier) {
        ...
        const scope = await this._getScopeContext();
        if (!scope.isMainAdmin) {
            // Non-main admins can only remove grants they personally created
            try {
                const { data, error } = await db
                    .from('admin_test_users')
                    .select('granted_by')
                ...
                if (this.normalizeIdentifier(data[0].granted_by) !== scope.currentEmail) {
                    return { ok: false, error: 'You can only remove grants you personally created.' };
                }
            } catch (e) {
                return { ok: false, error: 'Could not verify ownership of this grant.' };
            }
        }
        ...
        try {
            ...
            if (!error && data) {
                const filtered = scope.isMainAdmin
                    ? data
                    : data.filter(entry => this._matchesActor(entry.granted_by, scope.currentEmail));
                return mergeUsers(filtered, scope.isMainAdmin ? localUsers : []);
            }
        } catch (e) {
            console.warn('[Admin] Supabase list failed, using local:', e);
        }

        return mergeUsers([], scope.isMainAdmin ? localUsers : []);
    }
};
